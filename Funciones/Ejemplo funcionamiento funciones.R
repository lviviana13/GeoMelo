...
col_nbr <- poly2nb(columbus,queen=F)            # Torre
col.lags10 <- nblag(col_nbr, 10)             # Orden 10
col_nbq1 <- poly2nb(columbus)                   # Reina
col.lags <- nblag(col_nbq1, 10)

plot(columbus[col.lags10[[1]][[as.numeric(over(SpatialPoints(x),columbus)$ID)+1]],], col = "springgreen4", density=20, add = TRUE)
plot(columbus[col.lags10[[2]][[as.numeric(over(SpatialPoints(x),columbus)$ID)+1]],], col = "yellow4", density=20, add = TRUE)
...
centroids <- getSpPPolygonsLabptSlots(columbus[col.lags[[1]][[as.numeric(over(SpatialPoints(x),columbus)$ID)+1]],])
colnames(centroids) <- c("x","y")
plot(columbus[col.lags[[1]][[as.numeric(over(SpatialPoints(x),columbus)$ID)+1]],], col = "red", density=20, add = TRUE)
text(centroids[,1],centroids[,2],columbus[col.lags[[1]][[as.numeric(over(SpatialPoints(x),columbus)$ID)+1]],]$POLYID,cex=0.8)