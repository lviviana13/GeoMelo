spcorrelogram.bi <- function (neighbours, varX, varY, order = 1, method = "corr", style = "W", 
    randomisation = TRUE, zero.policy = NULL, spChk = NULL)
{
    ...
    if (method == "corr") {
        ...
        for (i in 1:order) {
            ...
            if (method == "I") {
                res[i, ] <-  moranbi1.test(x=varX,y=varY,listw,randomisation = randomisation, 
...
print.spcorbi <- function (x, p.adj.method="none", ...) 
{
    ...
    if (x$method == "I") {
        cat( "Bivariate Moran's I\n")       # Modif. PL
    } else if(x$method == "C") {
        cat( "Bivariate Geary's C\n")   # Modif. PL
    } else {
...
#        sd2 <- 2*sqrt(x$res[,3])
        ...
        if (missing(ylab)) 
            if(x$method == "I") ylab <- "Bivariate Moran's  I"            # Addition PL
            if(x$method == "C") ylab <- "Bivariate Geary's  C"            # Addition PL
